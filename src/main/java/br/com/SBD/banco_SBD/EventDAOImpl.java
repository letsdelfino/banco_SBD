...
public class EventDAOImpl implements EventDAO {
	...
	private final String SQL_GET_ACCOUNT =
	...
	private void prepareStatements() throws SQLException
	{
		...
		getAccountStatement =
			database.getConnection().prepareStatement(SQL_GET_ACCOUNT);
		getAllStatement =
	...
	{
		getAccountStatement.setInt(1, conta.getId());
		getAccountStatement.setInt(2, conta.getId());
		ArrayList<Event> list = new ArrayList<>();
		ResultSet set = getAccountStatement.executeQuery();
		while(set.next())
			list.add(getFromSet(set));
		return list;
	}
